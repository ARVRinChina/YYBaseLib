...
// 根据字体,大小,和label长度,获取label 的合适高度
#define FitHeight(string,font,originwidth) [string sizeWithFont:font constrainedToSize:CGSizeMake(originwidth,MAXFLOAT)].height

// 根据字体,大小,和label高度,获取label 的合适长度
#define FitWidth(string,font,originheight) [string sizeWithFont:font constrainedToSize:CGSizeMake(MAXFLOAT,originheight)].width
...
+ (void)autoShowWithMsg:(NSString *)msg customView:(UIView *)customView
{
    ...
    if(FitWidth(lbl.text, lbl.font, 20) > 220)
        [lbl setFrame:CGRectMake(0, 0, 220, FitHeight(lbl.text, lbl.font, 220))];
    else
        [lbl setFrame:CGRectMake(0, 0, FitWidth(lbl.text, lbl.font, 20), 20)];
...
#pragma mark - 添加的新方法，这样可以对其他的视图进行编辑

//添加的新方法，这样可以对其他的视图进行编辑
